...
static void lidar_task(void *arg) {
    ...
    while (1) {
        ...
        if (packet_idx == LD06_PACKET_LEN) {
            ...
            if (!crc_ok) {
                s_crc_errors++;
                /* Odrzucamy całą ramkę - dane (kąty/odległości poniżej) mogą
                 * być przekłamane przez błąd transmisji, więc nie chcemy ich
                 * pokazywać na dashboardzie (lidar_get_last_packet/_rpm/
                 * _min_distance) ani liczyć jako "OK" w diagnostyce. Wcześniej
                 * ten warunek chronił tylko bufor skanu (s_buf/s_polar), a
                 * s_last/s_ok_packets aktualizowały się bezwarunkowo - czyli
                 * dashboard mógł pokazać wartości z uszkodzonej ramki, a log
                 * diagnostyczny mylnie liczyć ją jako poprawną. */
                continue;
            }
            ...
            /* Bufor skanu i mapa biegunowa - ramka jest już zweryfikowana
             * (crc_ok gwarantowane przez wcześniejszy `continue`). */
            if (s_buf_mtx) {
