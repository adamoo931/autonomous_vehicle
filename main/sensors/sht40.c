...
/* CRC8 wg dokumentacji Sensirion: wielomian 0x31, wartość początkowa 0xFF. */
...
/* Wysyła jednobajtową komendę do czujnika. */
...
/* Odczytuje 6 bajtów wyniku: T(2)+CRC(1) oraz RH(2)+CRC(1). */
...
/* Pełny pomiar: komenda -> odczekanie -> odczyt 6 bajtów -> kontrola CRC ->
 * przeliczenie surowych wartości na jednostki fizyczne. */
static esp_err_t sht_measure(uint8_t addr, float *t, float *rh) {
    ...
    vTaskDelay(pdMS_TO_TICKS(10));   /* czas konwersji wysokiej precyzji      */
    ...
    *t  = -45.0f + 175.0f * ((float)t_raw  / 65535.0f);
    ...
    if (h < 0.0f)   h = 0.0f;     /* przycięcie wilgotności do zakresu 0-100 % */
...
esp_err_t sht40_init(void) {
    ...
    sht_send_cmd(SHT40_ADDR, SHT40_CMD_SOFT_RESET);
    vTaskDelay(pdMS_TO_TICKS(2));

    float t, rh;
    if (sht_measure(SHT40_ADDR, &t, &rh) == ESP_OK) {
        s_last.address       = SHT40_ADDR;
        s_last.temperature_c = t;
        s_last.humidity_pct  = rh;
        s_last.initialized   = true;
        ESP_LOGI(TAG, "SHT40 OK pod adresem 0x%02X (T=%.1f C, RH=%.1f%%)",
                 SHT40_ADDR, (double)t, (double)rh);
        return ESP_OK;
    }
    ESP_LOGE(TAG, "SHT40 nie wykryty pod 0x%02X (sprawdz magistrale I2C)", SHT40_ADDR);
