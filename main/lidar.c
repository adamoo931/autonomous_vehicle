...
static void lidar_task(void *arg) {
    ...
    while (1) {
        ...
        if (uart_read_bytes(LIDAR_UART_PORT, &byte, 1, pdMS_TO_TICKS(50)) != 1) continue;
        ...
        pkt.speed_rpm = (uint16_t)((buf[3] << 8) | buf[2]) / 100;
...
void lidar_init(void) {
    ...
    uart_driver_install(LIDAR_UART_PORT, 1024, 0, 0, NULL, 0);
    ...
    ESP_LOGI(TAG, "LIDAR (LD06) init OK – UART%d baud=%d",
             LIDAR_UART_PORT, LIDAR_BAUD_RATE);
