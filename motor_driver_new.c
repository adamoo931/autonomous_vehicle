...
/* Kompensacja asymetrii silników. Dwa nominalnie identyczne silniki DC z
 * przekładnią praktycznie nigdy nie kręcą się z tą samą prędkością przy
 * identycznym PWM (tarcie, luz przekładni, siła magnesów) - a nic w tym
 * projekcie nie koryguje kursu W TRAKCIE jazdy na wprost: korekta
 * (magnetometr, ST_ALIGN) działa tylko MIĘDZY odcinkami jazdy, więc pojazd
 * konsekwentnie skręca w stronę słabszego koła przez cały czas trwania
 * odcinka (do CRUISE_SEGMENT_MS=4000ms w autonomy.c), zanim cokolwiek to
 * zauważy. Efekt jest szczególnie widoczny przy SP_CRUISE blisko progu
 * tarcia statycznego (~35%) - ta sama różnica PWM przekłada się tam na dużo
 * większą różnicę prędkości niż w środku zakresu liniowego.
 *
 * DOSTRAJANIE: jedź na wprost na typowym SP_CRUISE przez kilka sekund po
 * płaskiej podłodze, zobacz w którą stronę skręca pojazd, i podnieś TRIM
 * PO STRONIE KOŁA, KTÓRE KRĘCI SIĘ ZA SZYBKO (zmniejsza jego moc o tyle
 * punktów procentowych). Wartości domyślne 0 = brak korekty - bezpieczne do
 * wgrania bez kalibracji, zachowują dotychczasowe zachowanie. */
#define MOTOR_TRIM_LEFT_PCT   0
#define MOTOR_TRIM_RIGHT_PCT  0

/* Zmniejsza moc o trim_pct w kierunku zera, zachowując znak (kierunek jazdy)
 * - trim działa tak samo do przodu i do tyłu, bo to własność mechaniczna
 * silnika/koła, niezależna od kierunku. */
static inline int apply_trim(int speed, int trim_pct) {
    if (speed > 0) return clamp(speed - trim_pct, 0, 100);
    if (speed < 0) return clamp(speed + trim_pct, -100, 0);
    return 0;
}
...
void motor_set_left(int speed) {
    ...
    speed   = apply_trim(speed, MOTOR_TRIM_LEFT_PCT);
    s_left  = speed;
...
void motor_set_right(int speed) {
    speed   = clamp(speed, -100, 100);
    speed   = apply_trim(speed, MOTOR_TRIM_RIGHT_PCT);
    s_right = speed;
